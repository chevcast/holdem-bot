...
export const builder = yargs => yargs
  ...
  .option("list", {
    description: "Show all the user's bankrolls across servers.",
    type: "boolean"
  });

export async function handler ({ discord, user: mention, list }) {
  const message = discord.message as Message;
  const userId = mention?.match(/^<@!?(\d+)>$/)?.[1] ?? message.author.id;
  if (message.channel.type === "dm" || list) {
    ...
    const accountsEmbed = new MessageEmbed()
      .setTitle(`${message.author.username}'s Bankrolls`)
      .setColor("#FDE15B")
